...
class ViewController: NSViewController {
    ...
    var checkKey = NSMenuItem(title: "禁用所有快捷键", action: #selector(switchKey), keyEquivalent: "")
    ...
    @objc func counter() {
        if (count >= 0) {
            ...
            hmsFrom(seconds: Int(count)) { hours, minutes, seconds in
                ...
                self.timeLabel.stringValue = "将在 "+"\(hours):\(minutes):\(seconds)"
            }
        } else {
    ...
    // Options menu
    func setUpMenu() {
        self.settingsMenu.addItem(NSMenuItem(title: "访问网站", action: #selector(openURL), keyEquivalent: ""))
        ...
        self.settingsMenu.addItem(NSMenuItem(title: "退出", action: #selector(NSApplication.terminate(_:)), keyEquivalent: "Q"))
